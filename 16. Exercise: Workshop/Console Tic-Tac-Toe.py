...
def read_player():
    ...
    while True:
        ...
        # "O" if first_player_sign == "X" else "X" comprehension
    second_player_sign = "O" if first_player_sign == "X" else "X"
    # funciata da vrushta dva instancii koito sa za suotvetniq igrach
...
def is_valid_postiion(board, mapper, position):
...
def has_won(board, sign):
    ...
    for row in board:
        ...
        # ["X", "X", "X"]
        # [True, True, True]
        # all fucntion proverqva dali vsichki stetment sa true
    ...
    # diagonals
    left_diaganol_win = True
    right_diaganol_win = True
    for idx in range(len(board)):
        # left diagonals
        if board[idx][idx] != sign:
            left_diaganol_win = False

        if board[idx][len(board) - 1 - idx] != sign:
            right_diaganol_win = False

    return left_diaganol_win or right_diaganol_win


first_player, second_player = read_player()

print(board_numeration())
...
while True:
    ...
    # proverqvame dali posiciqta e validna i da ne e X or O
    if not is_valid_postiion(board, board_mapper, position):
        continue
