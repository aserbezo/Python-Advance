...
# function checks if the row and col is outside and if is equal to * then count + 1
def count_neigh_bombs(game_field, row, col):
    count = 0
...
for row in range(game_field_size):
    for col in range(game_field_size):
        if game_field[row][col] == "*":
            continue
        cell_value = count_neigh_bombs(game_field, row, col)
